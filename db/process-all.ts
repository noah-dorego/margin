...
import { getDb, getDocument, updateDocumentStatus, insertFinding } from '../lib/db'
...
async function processDocument(id: string): Promise<void> {
  ...
  if (!doc) { console.error(`  NOT FOUND: ${id}`); return }

  console.log(`\nProcessing: "${doc.title}" (${doc.source_agency})`)
  ...
  try {
    ...
    for (const finding of extraction.findings) {
      try {
        ...
        console.error(`    FAIL: ${finding.finding_summary} — ${err instanceof Error ? err.message : String(err)}`)
        fail++
      }
    }
...
async function main() {
  const db = getDb()
  const pending = db
    .prepare(`SELECT id FROM regulatory_documents WHERE processing_status IN ('pending', 'failed')`)
    .all() as { id: string }[]
  ...
  const stats = db
    .prepare(`SELECT processing_status, COUNT(*) as count FROM regulatory_documents GROUP BY processing_status`)
  ...
  const findingCount = (db.prepare('SELECT COUNT(*) as n FROM regulatory_findings').get() as { n: number }).n
