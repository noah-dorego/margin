      ...
      {documents.length === 0 ? (
        ...
        >
          <p className="text-sm" style={{ color: 'var(--text-muted)' }}>
            ...
            <Link href="/documents/upload" style={{ color: 'var(--accent-blue)' }} className="hover:underline">
              Upload your first →
            </Link>
          </p>
        </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-subtle)' }}>
                {['Title', 'Agency', 'Date', 'Status', ''].map((h) => (
                  <th
                    key={h}
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-right">
                      {doc.processing_status === 'processed' ? null : (
                        <ProcessButton
                          documentId={doc.id}
                          label={doc.processing_status === 'failed' ? 'Retry' : 'Run'}
                        />
                      )}
                    </td>
                  </tr>
