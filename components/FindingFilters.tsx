...
export function FindingFilters() {
  ...
  const activeSeverities = (searchParams.get('severity')?.split(',') ?? []) as Severity[]
  const activeAgency     = searchParams.get('source_agency') ?? ''
  const activeProduct    = searchParams.get('product') ?? ''

  const hasFilters = activeSeverities.length > 0 || activeAgency || activeProduct
  ...
  return (
    ...
    >
      ...
      <FilterSection label="Agency">
        <select
          value={activeAgency}
          onChange={(e) => update('source_agency', e.target.value || null)}
